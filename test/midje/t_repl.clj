...
;;; then by creation order within namespace.
 (add-fact :midje/namespace 'midje.parsing.facts :midje/name "f - second - midje.parsing.facts")
 ...
 (fact :check-only-at-load-time
   ...
   (let [expected ["f - second - midje.parsing.facts"]]
     (names (fetch-facts 'midje.parsing.facts)) => expected
     (names (fetch-facts)) => expected)
   ...
   (let [expected ["z - first - integration" "f - second - midje.parsing.facts" "p - third - midje.repl" "m - fourth - midje.t-repl" "a - fifth - midje.t-repl"]]
     (names (fetch-facts :all)) => expected
     (names (fetch-facts)) => expected)

   (let [expected ["z - first - integration" "f - second - midje.parsing.facts"]]
     (names (fetch-facts :all "n")) => expected
     (names (fetch-facts)) => expected)
   
   (names (fetch-facts 'midje.parsing.facts #"q")) => []
   ...
   (names (fetch-facts "midje.parsing.facts" "p - third - midje.repl" :all))
   => ["f - second - midje.parsing.facts" "p - third - midje.repl"]
   )
 ...
 (without-changing-cumulative-totals
  (fact "interactions between forget-facts and other functions"
    ...
    (fact "it can get its default from load-facts"
      (forget-facts :all)
      (add-fact :midje/namespace 'midje.parsing.facts :midje/name "not forgotten")
    ...
    (fact "forget-facts does not affect check-facts"
      ...
      (add-fact :midje/namespace 'midje.parsing.facts)
      (forget-facts 'midje.parsing.facts)
      (map fact-name (fetch-facts)) => ["a non-featherian test"]))
  )
